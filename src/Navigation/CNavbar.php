    ...
    public function create()
    {
        ...
        $createMenu = function($items, $callback) use (&$createMenu, $createUrl) 
        {
          ...
          foreach ($items as $item) {
            ...
            $selected = ($selected || $selectedParent) ? "${selected}{$selectedParent}" : null;      
            $url = $createUrl($item['url']);

            if ($item['url'] == 'unclickable') {
              $html .= "\n<li class='{$selected} unclickable'><a href='{$url}' onclick='return false;' title='{$item['title']}'>{$item['text']}</a>{$submenu}</li>\n";
            } else {
              $html .= "\n<li class='{$selected}'><a href='{$url}' title='{$item['title']}'>{$item['text']}</a>{$submenu}</li>\n";
            }
          }
